...
import { useProfileEditor } from '@/composables/profile/useProfileEditor'
import {
  User,
  Mail,
  MapPin,
...
const {
  ...
  addEducation,
  removeEducation,
} = useProfileEditor(userStore, profile, skills, experiences, education, socialLinks)
    ...
    <section
      ...
      <div v-if="!isEditing" class="flex flex-col md:flex-row items-center gap-8">
        ...
        <div class="flex-1 text-center md:text-left">
          ...
          <div class="flex flex-wrap justify-center md:justify-start gap-4 text-sm text-gray-600 dark:text-zinc-400">
        ...
        <div class="flex flex-col md:flex-row gap-8">
          ...
          <div class="flex flex-col items-center gap-4">
            <div @click="triggerFileUpload"
              ...
              <img :src="editForm.avatar || userStore.getAvatar()" alt="Preview"
          ...
          <div class="flex-1 grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="md:col-span-2 space-y-2">
              ...
              <textarea v-model="editForm.bio" rows="3"
                class="w-full px-4 py-2.5 rounded-xl bg-gray-50 dark:bg-zinc-800/50 border border-gray-100 dark:border-zinc-700 focus:outline-none focus:ring-2 focus:ring-blue-500/50 dark:text-zinc-100 transition-all resize-none"></textarea>
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <section
        ...
        <div class="flex items-center justify-between mb-6">
          <h2 class="text-xl font-bold text-gray-800 dark:text-zinc-100 flex items-center gap-2">
            ...
            技能掌握
            <span v-if="isEditing" class="text-xs font-normal text-blue-500 animate-pulse ml-2">(拖动调整)</span>
          </h2>
          <button v-if="isEditing" @click="addSkill"
        ...
        <div class="space-y-5">
          <div v-for="(skill, index) in isEditing ? editSkills : skills" :key="skill.name"
            class="space-y-2 group/skill">
            <div class="flex justify-between text-sm items-center">
              <input v-if="isEditing" v-model="editSkills[index].name"
              ...
              <div class="flex items-center gap-2">
                <span class="font-bold tabular-nums transition-colors duration-300 text-sm"
                  :class="isEditing ? 'text-blue-500' : 'text-gray-500 dark:text-zinc-500'">
                  {{ skill.level }}%
                </span>
                <button v-if="isEditing" @click="removeSkill(index)"
            ...
            <div class="relative h-3 w-full bg-gray-100 dark:bg-zinc-800 rounded-full transition-all duration-300"
              :class="[
                isEditing
              ...
              <!-- 进度显示 -->
              <div class="absolute h-full rounded-full transition-all duration-300 ease-out"
                :class="[skill.color, isEditing ? 'brightness-110' : '']" :style="{ width: `${skill.level}%` }"></div>

              <!-- 编辑模式下的交互层 -->
              <input v-if="isEditing" v-model.number="editSkills[index].level" type="range" min="0" max="100"
                class="absolute inset-0 w-full h-full opacity-0 cursor-ew-resize z-10 appearance-none" />

              <!-- 编辑模式下的装饰轨道 -->
              <div v-if="isEditing"
                class="absolute inset-0 border border-dashed border-blue-400/50 rounded-full pointer-events-none"></div>
            </div>
          </div>
        </div>
        ...
        <div class="mt-8 pt-8 border-t border-gray-100 dark:border-zinc-800">
          ...
          <div class="flex flex-col gap-3">
            <div v-for="(social, index) in isEditing ? editSocialLinks : socialLinks" :key="social.platform"
              class="flex items-center gap-3 group/social">
              <div class="p-2.5 rounded-xl transition-all duration-300" :class="[
              ...
              <div class="flex-1 min-w-0">
                <div v-if="!isEditing" class="text-sm font-bold text-gray-700 dark:text-zinc-300 truncate">
                  {{ social.platform }}
                </div>
                <input v-if="isEditing" v-model="editSocialLinks[index].url" placeholder="URL..."
      ...
      <div class="lg:col-span-2 space-y-6">
        ...
        <section
          ...
          <div v-if="(isEditing ? editExperiences : experiences).length > 0"
            ...
            <div v-for="(exp, index) in isEditing ? editExperiences : experiences" :key="index"
              ...
              <button v-if="isEditing" @click="removeExperience(index)"
              ...
              <div class="flex flex-col sm:flex-row sm:items-center justify-between mb-2 gap-2">
                <input v-if="isEditing" v-model="editExperiences[index].role"
                ...
                <input v-if="isEditing" v-model="editExperiences[index].period"
              ...
              <div class="mb-2">
                <input v-if="isEditing" v-model="editExperiences[index].company"
              ...
              <textarea v-if="isEditing" v-model="editExperiences[index].desc" rows="2"
                class="w-full text-sm bg-transparent border border-blue-500/30 rounded-lg p-2 focus:border-blue-500 outline-none text-gray-500 dark:text-zinc-500 resize-none"></textarea>
        ...
        <section
          ...
          <div v-if="(isEditing ? editEducation : education).length > 0" class="space-y-6">
            <div v-for="(edu, index) in isEditing ? editEducation : education" :key="index"
              ...
              <div class="flex-1">
                <div class="flex flex-col sm:flex-row sm:items-center justify-between mb-1 gap-2">
                  <input v-if="isEditing" v-model="editEducation[index].school"
                    class="text-lg font-bold bg-transparent border-b border-blue-500/30 focus:border-blue-500 outline-none text-gray-800 dark:text-zinc-100 w-full sm:w-auto" />
                  <h3 v-else class="text-lg font-bold text-gray-800 dark:text-zinc-100">
                    {{ edu.school }}
                  </h3>

                  <input v-if="isEditing" v-model="editEducation[index].period"
                ...
                <div class="flex items-center gap-2">
                  <input v-if="isEditing" v-model="editEducation[index].major"
                  ...
                  <input v-if="isEditing" v-model="editEducation[index].degree"
              ...
              <button v-if="isEditing" @click="removeEducation(index)"
