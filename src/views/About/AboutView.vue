...
const { aboutContent, techStack, profile, socialLinks } = storeToRefs(userStore)
  ...
  <div class="space-y-6 pb-12 animate-fade-in-up">
    <section
      ...
      <div class="flex items-center gap-3 mb-6">
        ...
        <input v-if="isEditing" v-model="editedContent.title"
      ...
      <div class="prose dark:prose-invert max-w-none text-gray-600 dark:text-zinc-400 leading-relaxed">
        <!-- Introduction -->
        <div class="mb-6">
          <div v-if="isEditing" class="space-y-2">
            <label class="text-xs font-medium text-gray-400 uppercase tracking-wider">个人简介 (支持 HTML)</label>
            <textarea v-model="editedContent.introduction" rows="3"
              class="w-full p-3 rounded-xl bg-gray-50 dark:bg-zinc-800/50 border border-gray-200 dark:border-zinc-700 focus:border-blue-500 outline-none transition-all resize-none"></textarea>
          </div>
        ...
        <!-- Description -->
        <div class="mb-4">
          <input v-if="isEditing" v-model="editedContent.description"
        ...
        <div class="space-y-3">
          <div v-if="isEditing" class="space-y-2">
            <div v-for="(point, index) in editedContent.points" :key="index" class="flex items-center gap-2 group">
              <div class="w-1.5 h-1.5 bg-blue-500 rounded-full shrink-0"></div>
              <input v-model="editedContent.points[index]"
              ...
              <button @click="removePoint(index)"
          ...
          <ul v-else class="list-none space-y-3 ml-4">
            <li v-for="(point, index) in aboutContent.points" :key="index" class="flex items-start gap-3">
      ...
      <div
        ...
        <div v-if="isEditing" class="space-y-4">
          ...
          <div class="flex gap-2">
            <input v-model="newTech" @keyup.enter="addTech" placeholder="添加新技能..."
          ...
          <div class="flex flex-col gap-2 max-h-[300px] overflow-y-auto pr-2 custom-scrollbar">
            <div v-for="(skill, index) in editedTechStack" :key="index" draggable="true" @dragstart="onDragStart(index)"
              ...
              <GripVertical class="w-4 h-4 text-gray-400 group-hover:text-blue-500 transition-colors" />
              <input v-model="editedTechStack[index]" class="flex-1 text-sm bg-transparent border-none outline-none" />
              <button @click="removeTech(index)"
        ...
        <div v-if="isEditing" class="space-y-4">
          ...
          <div class="space-y-1">
            ...
            <div
              ...
              <input v-model="editedProfile.email" class="flex-1 text-sm bg-transparent border-none outline-none"
                placeholder="您的邮箱" />
            </div>
          </div>
            ...
            <div v-for="(link, index) in editedSocialLinks" :key="index"
              ...
              <div class="flex items-center justify-between">
                <div class="flex items-center gap-2">
                  ...
                  <input v-model="link.platform"
                    class="text-xs font-bold bg-transparent border-none outline-none w-24" />
                </div>
                <button @click="removeSocial(index)" class="p-1 text-gray-400 hover:text-rose-500 transition-colors">
                  <X class="w-4 h-4" />
                </button>
              </div>
        ...
        <ul v-else class="space-y-4 text-gray-600 dark:text-zinc-400">
          <li
            ...
            <div class="flex flex-col">
              ...
              <a :href="'mailto:' + profile.email" class="text-sm font-medium hover:text-blue-500 transition-colors">
                {{ profile.email }}
              </a>
            </div>
          </li>
