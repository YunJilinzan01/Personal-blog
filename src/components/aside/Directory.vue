...
import { useDirectoryManager } from '@/composables/aside/useDirectoryManager'
  ...
  <div
    ...
    <Transition name="fade">
      <div v-if="isEditing" class="mb-4 flex gap-2 animate-fade-in-up">
        <input v-model="newCategoryName" type="text" placeholder="新增目录..." @keyup.enter="addNewCategory"
    ...
    <div class="flex flex-wrap gap-2">
      <TransitionGroup name="list">
        <div v-for="(cat, index) in blogStore.categories" :key="cat.name" :draggable="isEditing"
          ...
          <button v-if="isEditing" @click.stop="removeCategory(index)"
    ...
    <div v-if="blogStore.categories.length === 0"
      class="py-6 text-center text-sm text-gray-400 dark:text-zinc-500 italic">
      暂无目录
    </div>
  </div>
</template>
