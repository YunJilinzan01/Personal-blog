<script setup>
import { useProjectList } from '@/composables/aside/useProjectList'
import { Plus, Trash2, FolderOpen } from 'lucide-vue-next'

const { projects, isAdding, newProject, addProject, removeProject } = useProjectList()
</script>
  ...
  <div
    ...
    <div class="flex items-center justify-between mb-4">
      ...
      <button @click="isAdding = !isAdding"
        ...
        :class="{ 'bg-[#37a8ec] text-white rotate-45': isAdding }" title="添加项目">
    ...
    <Transition enter-active-class="transition duration-300 ease-out"
      ...
      <div v-if="isAdding"
        ...
        <div class="space-y-3">
          <input v-model="newProject.title" type="text" placeholder="项目名称"
          ...
          <input v-model="newProject.link" type="text" placeholder="项目链接 (Github/Demo)"
            class="w-full px-3 py-2 bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-700 rounded-lg text-sm focus:outline-none focus:border-[#37a8ec] dark:text-zinc-200 transition-colors" />
          <input v-model="newProject.tags" type="text" placeholder="标签 (多个请用逗号分隔)"
            class="w-full px-3 py-2 bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-700 rounded-lg text-sm focus:outline-none focus:border-[#37a8ec] dark:text-zinc-200 transition-colors" />
          <textarea v-model="newProject.description" placeholder="项目描述..." rows="2"
            class="w-full px-3 py-2 bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-700 rounded-lg text-sm focus:outline-none focus:border-[#37a8ec] dark:text-zinc-200 transition-colors resize-none"></textarea>
    ...
    <div class="flex flex-col gap-4">
      ...
      <div v-if="projects.length === 0"
        ...
        <div class="p-3 bg-gray-50 dark:bg-zinc-800/50 rounded-full mb-3 text-zinc-400 dark:text-zinc-500">
          <FolderOpen :size="24" />
        </div>
      ...
      <div v-for="(project, index) in projects" :key="project.id"
        ...
        <div class="group pr-8">
          <a :href="project.link" target="_blank"
            ...
            {{ project.title }}
            <span class="text-xs text-zinc-400 group-hover:translate-x-1 transition-transform duration-300">→</span>
          </a>
          <p class="text-sm text-zinc-500 dark:text-zinc-400 mt-1 line-clamp-2 leading-relaxed">
            {{ project.description }}
          </p>
          <div class="flex gap-2 mt-2">
            <span v-for="tag in project.tags" :key="tag" class="text-[10px] px-2 py-0.5 rounded-md transition-colors"
        ...
        <!-- 分割线 (最后一个项目后不显示) -->
        <div v-if="index !== projects.length - 1" class="h-px bg-gray-100 dark:bg-zinc-800/50 w-full mt-4"></div>
      </div>
    </div>
  </div>
</template>
