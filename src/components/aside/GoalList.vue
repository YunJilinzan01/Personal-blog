<script setup>
import { useGoalList } from '@/composables/aside/useGoalList'

const { goals, newGoalTitle, addGoal, toggleGoal, removeGoal, resetGoals } = useGoalList()
</script>
...
<template>
  <div class="transition-colors duration-300">
    <div class="flex items-center justify-between mb-4">
      ...
      <button @click="resetGoals" class="p-1.5 text-gray-400 hover:text-red-500 transition-colors cursor-pointer"
        title="重置所有目标">
    ...
    <div class="flex gap-2 mb-6">
      <input v-model="newGoalTitle" @keyup.enter="addGoal" type="text" placeholder="添加新目标..."
    ...
    <div class="space-y-3 max-h-60 overflow-y-auto pr-1 custom-scrollbar">
      <TransitionGroup name="list">
        <div v-for="goal in goals" :key="goal.id" class="flex items-start gap-3 group cursor-pointer"
          ...
          <div class="mt-1 shrink-0">
            ...
            <div v-else class="text-gray-300 dark:text-zinc-600 group-hover:text-blue-400 transition-colors">
          ...
          <!-- 删除按钮 -->
          <button @click.stop="removeGoal(goal.id)"
